  ...
  useEffect(() => {
    ...
    if (!newLocalStorage.get("firstCome")) {
      ...
      if (times > 60000) {
        speak(
          `哎哟，你回来了啊！与你上次相见有${
            times > 60000 && times < 60000 * 30
              ? Math.ceil(times / 60000) + "分钟"
              : times > 60000 * 30 && times < 60000 * 60
              ? "超过半个小时"
              : times > 60000 * 60 && times < 60000 * 60 * 7
              ? "好几个小时"
              : times > 60000 * 60 * 12 && times < 60000 * 60 * 36
              ? "接近一天"
              : Math.ceil(times / (60000 * 60 * 24)) + "天多"
          }了， 最近有什么新鲜事吗？`,
          7
        );
      }
                        ...
                        {noticeList.map((item, index) => (
                          <li className="pa24" key={item.id}>
                            <div className="flexB">
                              ...
                              <IonIcon
                                onClick={() =>
                                  handleDeleteNotice(item.id, index)
                                }
                                icon={trashOutline}
                                className="font16"
                              />
